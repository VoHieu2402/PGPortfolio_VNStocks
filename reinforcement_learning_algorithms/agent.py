...
class Agent(nn.Module):
  """
      ...
      "prev_pf_for_loss" -> (batch_size, lags_for_sharpe)
      "prev_action_pf" -> (batch_size, num_stocks+1, 1)
      "prev_bm" -> (batch_size, 1)
      "prev_bm_for_loss" -> (batch_size, lags_for_sharpe)
      "pre_each_asset" -> (batch_size, num_stocks+1)
  """
  def __init__(self, num_features: int=3, num_stocks: int=5, lags: int=30, out_cv1: int=2, out_cv2: int=30, kernel_size_cv1: int=3, dropout_rate: float=0.2, lr_actor: float=0.001, rb_capacity: int=100, batch_size: int=30, gamma: float=0.9, lags_for_sharpe: int=48, trans_cost: float=0.0025, rebalance_period: int=3):
    super().__init__()
    ...
    self.lags_for_sharpe = lags_for_sharpe
    self.gamma = gamma
  ...
  def train(self, update_lr=False, print_result=False):
    if len(self.replay_buffer) < self.batch_size + self.lags_for_sharpe:
          return
    transitions = self.replay_buffer.sample(self.batch_size, self.lags_for_sharpe)

    state_portfolio = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_features, self.num_stocks, self.lags, dtype=torch.float32
    )
    next_state_portfolio = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_features, self.num_stocks, self.lags, dtype=torch.float32
    )
    action = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_stocks, 1, dtype=torch.float32
    )
    reward = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_stocks, 1, dtype=torch.float32
    )
    ...
    state_benchmark = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_features, 1, self.lags, dtype=torch.float32
    )
    next_state_benchmark = torch.zeros(
        self.batch_size+self.lags_for_sharpe, self.num_features, 1, self.lags, dtype=torch.float32
    )
    prev_pf = torch.zeros(
        self.batch_size+self.lags_for_sharpe, 1, dtype=torch.float32
    )
    ...
    for i in range(self.batch_size+self.lags_for_sharpe):
    ...
    state_portfolio = state_portfolio[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    next_state_portfolio = next_state_portfolio[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    action = action[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    reward = reward[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    prev_action = prev_action[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    state_benchmark = state_benchmark[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    next_state_benchmark = next_state_benchmark[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    pre_each_asset = pre_each_asset[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    

    # Calculate the historical ret for portfolio
    prev_pf_ = prev_pf[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    ...
    for n in range(self.batch_size):
      for l in range(self.lags_for_sharpe-1):
        prev_pf_for_loss[n,l] = prev_return_pf[n+l]
        
    # Calculate the historical ret for benchmark
    prev_bm_ = prev_bm[self.lags_for_sharpe:].clone().detach().requires_grad_(False)
    ...
    # Define boolean masks for rows divisible and not divisible by rebalance period
    mask_divisible = torch.arange(pre_each_asset.size(0)) % self.rebalance_period == 0
    
    # Re-calculate portfolio after new allocation
    post_each_asset_1 = action_portfolio.squeeze(2)[mask_divisible] * prev_pf_[mask_divisible]
    transaction_amount = post_each_asset_1 - pre_each_asset[mask_divisible]
    transaction_cost_for_each = torch.abs(transaction_amount) * self.trans_cost
    post_each_asset_1[:,-1] -= torch.sum(transaction_cost_for_each, 1) - transaction_cost_for_each[:,-1]
    new_prev_pf_1 = torch.sum(post_each_asset_1, 1)
    prev_cash_1 = post_each_asset_1[:,-1].unsqueeze(1)

    action_portfolio_2 = (pre_each_asset[~mask_divisible] / prev_pf_[~mask_divisible])
    post_each_asset_2 = action_portfolio_2 * prev_pf_[~mask_divisible]
    new_prev_pf_2 = torch.sum(post_each_asset_2, 1)
    prev_cash_2 = post_each_asset_2[:,-1].unsqueeze(1)

    new_prev_pf_ = torch.sum(pre_each_asset, 1)
    new_prev_pf_[mask_divisible] = new_prev_pf_1
    new_prev_pf_[~mask_divisible] = new_prev_pf_2

    # Calculate the reward of the action - Daily return
    ret = 1 / next_state_portfolio[:,0,:,-2].unsqueeze(2)
    tot_ret_1 = ret[mask_divisible] * post_each_asset_1[:,:self.num_stocks].unsqueeze(-1)
    new_pf_1 = torch.sum(tot_ret_1.squeeze(-1),-1).unsqueeze(-1) + post_each_asset_1[:,-1].unsqueeze(-1)
    tot_ret_2 = ret[~mask_divisible] * post_each_asset_2[:,:self.num_stocks].unsqueeze(-1)
    new_pf_2 = torch.sum(tot_ret_2.squeeze(-1),-1).unsqueeze(-1) + post_each_asset_2[:,-1].unsqueeze(-1)
    new_pf = torch.sum(pre_each_asset, 1).unsqueeze(-1)
    new_pf[mask_divisible] = new_pf_1
    new_pf[~mask_divisible] = new_pf_2
